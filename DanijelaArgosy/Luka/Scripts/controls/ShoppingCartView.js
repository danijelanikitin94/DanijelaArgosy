function ShoppingCartView(opts) {
    var that = this;
    ...
    controlLoader.loadTemplate("ShoppingCartView", function (template) {
        $(document.body).append(template);
        var html = $(that.options.ShoppingCartViewTemplate).html();
...
ShoppingCartView.prototype.options = {};

ShoppingCartView.prototype.baseOptions = {
    shoppingCartBuilderView: "div[data-argosy-control=ShoppingCartView]",
    ShoppingCartViewTemplate: "#_ShoppingCartViewTemplate",
    shoppingCartBindSection: "#_ShopingCartBindSection",
    shoppingCartGridHref: "#_ShoppingCartGrid",
    shoppingCartGridRowTempalte: "#_ShoppingCartRowViewTemplate",
    cartContainer: "#_cartContainer",
    cartSubTotalSelector: "._ShoppingCartSubTotal",
...
ShoppingCartView.prototype.EVENT_TEMPLATE_LOADED = "TEMPLATE_SHOPPING_CART_VIEW_LOADED";
ShoppingCartView.prototype.productDetailControl = null;
ShoppingCartView.prototype.cartViewModel = null;
ShoppingCartView.prototype.setupLoadingAttributes = function () {
    var that = this;
    var mainElement = $("*[data-argosy-uuid='" + that.options.uuid + "']");
    appendLoadingAttribute(mainElement, that.constructor.name);
    mainElement.attr("data-argosy-loading-message", "~{MsgUpdatingCart}~");

};
ShoppingCartView.prototype.setupEvents = function () {
    var that = this;
    $(document).bind(argosyEvents.EVENT_SAVED_CARTLINE_ADDED_TO_SHOPPING_CART, function (e) {
...
ShoppingCartView.prototype.parts = [];

ShoppingCartView.prototype.getPart = function (partId) {
    var that = this,
        part = null;
...
ShoppingCartView.prototype.initialze = function () {
    var that = this,
            ...
            requestEnd: function (e) {
                ...
                if (e.hasOwnProperty("response")) {
                    ...
                    $.each(e.response.CartLines, function () {
                        ...
                        if (line.ParentPartId > 0 && line.ParentPartId !== line.PartId && $.grep(parentPartIds, function (parentPartId) {
                            return parentPartId === line.ParentPartId;
                        }).length === 0) {
                    ...
                    if (parentPartIds.length > 0) {
                        that.cartViewModel.getRemoteParts(parentPartIds, function (parts) {
                            $(parts).each(function (i, part) {
                                that.parts.push(part);
                                partIds.push(part.PartId);
                                var configuredParts = $.grep(e.response.CartLines, function (line) {
                                ...
                                var parentCartLine = $.extend({}, configuredParts[0], {
                                    Part: part,
                                    CartId: 0,
                                    PartId: part.PartId,
                                    IsKit: false,
                                ...
                                $(configuredParts).each(function (x, line) {
                                    parentCartLine.Price += line.Price;
                                    parentCartLine.TotalConfiguredParts += line.Quantity;
                                    parentCartLine.Discount += line.Discount;
                                });
                                e.response.CartLines = $.grep(e.response.CartLines, function (line) {
                                    return line.ParentPartId !== part.PartId;
                                });
    ...
    that.cartViewModel = kendo.observable({
        ...
        editAttributes: function (e) {
            var part = e.data.Part,
                attributePartId = e.data.AttributePartId,
                values = e.data.CartAttributes,
                ...
                model = kendo.observable({
                    ...
                    getAttributeCostText: function (e, d) {
                        //debugger;
                    },
                    updateAttributes: function (e) {
                        var partsToAdd = [],
                            attributes = $(".fancybox-inner").find("select.attributeOptions,input.attributeOptions");
                        if (attributes.length > 0) {
                            ...
                            $(attributes).each(function(i, attribute) {
                                ...
                                if (attribute.is("select")) {
                                    ...
                                    selectedAttributes.push({
                                        ...
                                        Name: attribute.val(),
                                        Type: attribute.attr("data-type-name")
                                    });
                                } else if (isRequired) {
                                    isValid = false;
                                }
                            ...
                            $.ajax({
                                url: "/store/cart/UpdateCartLineAttributes",
                                type: "POST",
                                dataType: "json",
                                data: JSON.stringify({
            ...
            $.ajax({
                url: "/Store/Part/GetPartAttributeConfiguration/" + attributePartId,
                type: "GET",
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (attributeGroups) {
                    ...
                    var content = renderEditAttributesModal(model.part, attributeGroups, model.values);
                    $.fancybox({
                        content: content,
                        afterShow: function () {
        ...
        onDataProcessComplete: function (data, partIds) {
            ...
            that.cartViewModel.getCrossSaleItems();
        ...
        someAreEdelivery: function (data) {
            return $.grep(this.cartLines, function (line) {
                return line.Part.IsEdeliveryAllowed || line.Part.IsEdeliveryOnly;
            }).length > 0;
        },
        ...
        doesLineNeedAttention: function (e) {
            if (e.CartStatus !== 99 || e.ShowOverrideMessage === true) {
                return true;
            }
            return false;
        },
            ...
            $.ajax({
                ...
                complete: function (e) {
                    ...
                    if (response.isError === false) {
                        var elementName = response.postUsingUrlEncoded ? "cXML-urlencoded" : "cXML-base64";
                        $.submitAsForm(response.postUrl, response.xml, elementName);
                    } else {
                        var message = {
                            question: "Problem with checkout.",
        ...
        checkout: function (e) {
            var kendoObj = this,
                ...
                approvalMessage = kendoObj.get("approvalWorkflowWarningMsg");

            $.each(cartLines, function (e) {
                this.DateCreated = kendoObj.dateCleaner(this.DateCreated);
                this.DateUpdated = kendoObj.dateCleaner(this.DateUpdated);
            });

            var mainElement = $("*[data-argosy-uuid='" + that.options.uuid + "']");
            mainElement.attr("data-argosy-loading-message", "~{MsgVerifyingCart}~");
            $(document).trigger(argosyEvents.START_LOADING, {
                name: that.constructor.name
            });
            var submitLines = kendoObj.flattenCartLines(cartLines);

            $.ajax({
                url: "/store/cart/CartReadyForCheckout",
                dataType: "json",
                type: "POST",
                data: JSON.stringify(submitLines),
                contentType: "application/json; charset=utf-8",
                complete: function (e) {
                    $(document).trigger(argosyEvents.END_LOADING, {
                        name: that.constructor.name
                    ...
                    mainElement.attr("data-argosy-loading-message", "~{MsgUpdatingCart}~");
                    kendoObj.isFinalCalculation = true;
                    var shoppingCart = e.responseJSON;
                    kendoObj.set("approvalWorkflowWarningMsg", shoppingCart.ApprovalWarningMessage);
                    kendoObj.set("showWarningAboutApprovalWorkflow", shoppingCart.ShowWarningAboutApprovalWorkflow);
                    kendoObj.set("redirectUrl", shoppingCart.ReturnUrl);
                    kendoObj.set("redirectMethod", function () {
                        if (userSettings.IsCXmlPunchOut) {
                            kendoObj.postCxml();
                        } else {
                            if (shoppingCart.ReturnUrl.indexOf("placeorder") > 0) {
                                window.location = shoppingCart.ReturnUrl;
                            } else {
                                $.submitAsForm(shoppingCart.ReturnUrl + "?redirectUrl=/Store/Cart");
                            }
                        }
                    });
                    approvalMessage = shoppingCart.ApprovalWarningMessage;
                    if (shoppingCart.CartLines && shoppingCart.CartLines.length > 0) {
                        kendoObj.updateCartLines(cartLines, shoppingCart.CartLines, true);
                        kendoObj.setCartLines(cartLines, true);
                    }
                    if (shoppingCart.ReturnCode !== 200) {
                        var promptData = {
                            message: "~{MsgUnableToCheckout}~",
                            type: "error"
                        };
                        $(document).trigger(argosyEvents.PROMPT_NOTIFY, promptData);
                    } else {
                        var showModal = false,
                            divApprovalWorkflowMessages = $("div[data-id='approvalWorkflowMessages']"),
                            divLineMessages = $("div[data-id='lineMessages']"),
                            divLineErros = $("div[data-id='lineErrors']"),
                            diveMoreInfo = $("div[data-id='moreInformation']");
                        if (shoppingCart.Response !== null) {
                            if (shoppingCart.Response.ApprovalWorkflowMessages.length > 0) {
                                showModal = true;
                                var approvalWorkflowMessagesList =$("div[data-id='approvalWorkflowMessagesList']").data("kendoListView");

                                $.each(shoppingCart.Response.ApprovalWorkflowMessages,
                                    function(i, item) {
                                        if (item !== "~{MsgStopOrderReady}~") {
                                            approvalWorkflowMessagesList.dataSource.add({ Message: item });
                                        };
                                    });
                                divApprovalWorkflowMessages.show();
                                kendoObj.set("lineErrorsHrVisible", true);
                                kendoObj.set("lineMessagesHrVisible", true);
                            } else {
                                divApprovalWorkflowMessages.hide();
                                kendoObj.set("lineErrorsHrVisible", false);
                                kendoObj.set("lineMessagesHrVisible", false);
                            };

                            if (shoppingCart.Response.LineMessages.length > 0) {
                                showModal = true;
                                var lineMessagesGrid = $("div[data-id='lineMessagesGrid']").data("kendoGrid");
                                $.each(shoppingCart.Response.LineMessages,
                                    function(i, item) {
                                        lineMessagesGrid.dataSource.add(item);
                                    });
                                divLineMessages.show();
                                kendoObj.set("lineErrorsHrVisible", true);
                            } else {
                                divLineMessages.hide();
                                kendoObj.set("lineErrorsHrVisible", false);
                            };

                            if (shoppingCart.Response.LineErrors.length > 0) {
                                showModal = true;
                                var lineErrorsGrid = $("div[data-id='lineErrorsGrid']").data("kendoGrid");
                                $.each(shoppingCart.Response.LineErrors,
                                    function(i, item) {
                                        lineErrorsGrid.dataSource.add(item);
                                    });
                                kendoObj.set("alertMessageContinueEnabled", false);
                                diveMoreInfo.show();
                                divLineErros.show();
                            } else {
                                diveMoreInfo.show();
                                kendoObj.set("alertMessageContinueEnabled", true);
                                divLineErros.hide();
                            };
                        }
                        $("#_alertMessageContinueToCheckout").on("click", function (e) {
                            $.fancybox.close();
                            mainElement.attr("data-argosy-loading-message", "~{MsgHeadingToCheckout}~");
                            $(document).trigger(argosyEvents.START_LOADING, {
                                name: that.constructor.name
                            });
                            kendoObj.redirectMethod();
                        });

                        if (showModal) {
                            $.fancybox({
                                href: "#cartResponseSection",
                                afterShow: function() {
                                    $(document).trigger(argosyEvents.END_LOADING, {
                                        name: that.constructor.name
                                    });
                            ...
                            $("div[data-id='cartResponseSection']").hide();
                            if (shoppingCart.Response.Status === 99) {
                                kendoObj.redirectMethod();
                            }else if (shoppingCart.Response.Status === 90) {
                                var message = {
                                    question: "~{MsgFixHighlighted}~",
                                    description: shoppingCart.Message,
                                    button: "Close",
                                    type: "warning",
                                }
                                prompt.alert(message);
        ...
        hiddenIfNoCartAttributes: function (e) {
            if (e.CartAttributes !== undefined && e.CartAttributes !== null && e.CartAttributes.length > 0) {
                return false;
            } else {
                return true;
            }
        ...
        saveForLater: function (line) {
            var kendoObj = this,
                cartLine = line.data;
            var mainElement = $("*[data-argosy-uuid='" + that.options.uuid + "']");
            mainElement.attr("data-argosy-loading-message", "~{MsgSavingCartForLater}~");
            $(document).trigger(argosyEvents.START_LOADING, {
                name: that.constructor.name
            });
            ...
            $.ajax({
                url: "/store/cart/SaveCartlineForLater",
                dataType: "json",
                type: "POST",
                data: JSON.stringify({
                    lines: cartLines
                ...
                complete: function (e) {
                    that.cartViewModel.cartSource.read();
                    $(document).trigger(argosyEvents.EVENT_CARTLINE_SAVED_FOR_LATER);
                    $(document).trigger(argosyEvents.END_LOADING, {
                        name: that.constructor.name
                    });
                    mainElement.attr("data-argosy-loading-message", "~{MsgUpdatingCart}~");
            ...
            $.ajax({
                ...
                success: function (e) {
                    var success = e.ReturnCode === ReturnCode.Success,
                        message = "~{MsgItemDeletedFromCart}~",
                        type = "success";
                    if (!success) {
                        message = e.Message;
                        type = "error";
                    } else {
                        kendoObj.updateLine(line);
                    } 
                ...
                error: function () {
                    var promptData = {
                        message: "~{MsgErrorDeletingCartItem}~",
                        type: "error"
                    };
                    $(document).trigger(argosyEvents.PROMPT_NOTIFY, promptData);
                }
        ...
        setCartLines: function (lines, rebind) {
            ...
            var mailingListCost = 0;
            var thirdPartyListCost = 0;
            var discountTotal = 0;
        ...
        accountingUnitsChange: function (e) {
            ...
            $.each(cartLines, function (e, cartline) {
                if (cartline.CartId === cartId) {
                    ...
                    if (cartline.ChildCartLines) {
                        $.each(cartline.ChildCartLines, function (i, line) {
                            line.AccountingUnitId = unit;
                        });
                    }
                    $.ajax({
        ...
        getCrossSaleItems: function () {
            var kendoObj = this,
                partIds = kendoObj.get("partIds");

            if (partIds.length > 0) {
                $.ajax({
                    url: "/store/cart/GetCrossSaleItems",
                    dataType: "text json",
                    data: JSON.stringify(partIds),
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    success: function (e) {
                        if (e && e.length > 0) {
                            $(e).each(function (i) {
                                that.parts.push(this);
                            });
                            kendoObj.set("crossSaleItems", new kendo.data.ObservableArray(e));
                            $('#_cartCrossSaleBindSection').show();
                            addArgosyActions();
                        } else {
                            $(".toggle-recommend-controls").hide();
                        }
                    }
                });
            }
        ...
        updatePrice: function (lines) {
            var kendoObj = this,
                cartLines = lines == null ? kendoObj.get("cartLines") : lines,
                selectorForShowingLoading = $(".show-loading, [data-role=partpricing]");

            selectorForShowingLoading.html("Loading...");
            var submitLines = kendoObj.flattenCartLines(cartLines);
            if (cartLines.length > 0) {
                block(null, "Recalculating your cart...", true);
                $.ajax({
                    url: "/store/cart/UpdateCartlineUnitCosts?isFinalCalculation=" + kendoObj.isFinalCalculation,
                    dataType: "text json",
                    data: JSON.stringify(submitLines),
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    success: function (returnedLines) {
                        if (returnedLines && returnedLines.length > 0) {
                            kendoObj.updateCartLines(cartLines, returnedLines);
                            kendoObj.setCartLines(cartLines, true);
                        }
                        unblock();
                    }
        ...
        updateCartLines: function (cartLines, returnedLines, ignorePricing) {
            ...
            $.each(cartLines, function (e, line) {
                ...
                if (children.length > 0) {
                    var childSummary = that.updateCartLines(children, returnedLines);
                    line.Price = ignorePricing ? line.Price : childSummary.price;
                    line.Discount = ignorePricing ? line.Discount : childSummary.discount;
                    ...
                    if (result && result.length > 0) {
                        if (!ignorePricing) {
                            ...
                            parentDiscount += line.Discount;
                        }
                        line.CartAttributes = result[0].CartAttributes;
            ...
            return { price: parentPrice, discount: parentDiscount };
        },
        lineMessages: function() {
            return this.getLineMessages();
        },
        getLineMessages: function() {
        ...
        approvalWorkflowMessages: function() {
            return this.getApprovalWorkflowMessages();
        },
        getApprovalWorkflowMessages: function() {
            return new kendo.data.DataSource();
        },
        lineErrors: function() {
            return this.getLineErrors();
        },
        getLineErrors: function() {
            return new kendo.data.DataSource({
                schema: {
                    model: {
                        fields: {
        ...
        lineResponsesGroup: function(e) {
            $.fancybox.update();
        },
        lineResponsesDataBound: function (e) {
            var groups = $(".k-grouping-row");
            for (var i = 0; i < groups.length; i++) {
                e.sender.collapseGroup(groups[i]);
            };
            $.fancybox.update();
        },
        lineResponsesDataBinding: function (e) {
            var lineErrorsGrid = $("div[data-id='lineErrorsGrid']").data("kendoGrid"),
                lineMessagesGrid = $("div[data-id='lineMessagesGrid']").data("kendoGrid"),
                html = $("#_lineResponseRowTemplate").html(),
                template = kendo.template(html);
            if (lineErrorsGrid != null) {
                lineErrorsGrid.options.rowTemplate = $.proxy(template, lineErrorsGrid.dataSource);
            };
            if (lineMessagesGrid != null) {
                lineMessagesGrid.options.rowTemplate = $.proxy(template, lineMessagesGrid.dataSource);
            };
            $.fancybox.update();
        },
        alertMessageContinueToCheckout: function(e) {
            $.fancybox.close();
            var mainElement = $("*[data-argosy-uuid='" + that.options.uuid + "']");
            mainElement.attr("data-argosy-loading-message", "~{MsgHeadingToCheckout}~");
            $(document).trigger(argosyEvents.START_LOADING, {
        ...
        clearCartResponse: function (e) {
            ...
            e.data.set("approvalWorkflowMessages", []);
            e.data.set("cartMessages", e.data.getApprovalWorkflowMessages());
            e.data.set("lineMessages", e.data.getLineMessages());
            e.data.set("lineErrors", e.data.getLineErrors());
            e.data.set("lineErrorsHrVisible", false);
            e.data.set("lineMessagesHrVisible", false);
            $.fancybox.close();
        },
        lineErrorsHrVisible: false,
        lineMessagesHrVisible: false,
        toggleShowInformation: function (e) {
            $("div[data-id='moreInformation']").toggle();
            $.fancybox.update();
        }
    });
    kendo.bind($("#cartResponseSection"), that.cartViewModel);
...
ShoppingCartView.prototype.showCartIf = function (show) {
    var that = this;
    if (show) {
        $(that.options.shoppingCartGridHref + ",.hide-me-on-empty").removeClass('hidden');
        $(that.options.emptyCartSelector).addClass('hidden');
    } else {
        $(that.options.shoppingCartGridHref + ",.hide-me-on-empty").addClass('hidden');
        $(that.options.emptyCartSelector).removeClass('hidden');
    }
};
ShoppingCartView.prototype.getUserSettings = function () {
    var that = this;
    return new kendo.data.ObservableObject(window[that.options.userSettings]);
};
ShoppingCartView.prototype.setupEventListeners = function () {
    var that = this;
